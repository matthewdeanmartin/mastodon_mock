...
if (checkOnly) {
  ...
  if (JSON.stringify(membership(previous)) !== JSON.stringify(membership(report))) {
    console.error('Bundled starter-kit membership changed. Run npm run starter-kits:update and review the report.');
    process.exitCode = 1;
  } else {
    console.log(`Revalidated ${uniqueHandles.length} unique candidate accounts; shipped membership is unchanged.`);
  }
} else {
...
`;
  ...
  const unresolved = [...validations.values()].filter((value) => value.status === 'unresolved').length;
  console.log(
    `Updated 10 kits with ${included} memberships (${excluded} unique opt-outs, ${unresolved} unique unresolved).`,
  );
  for (const kit of kits) console.log(`  ${kit.title}: ${kit.accounts.length}/${candidates.find((item) => item.slug === kit.slug).handles.length}`);
}
