...
import { Chat, Conversations } from './conversations';
...
describe('Conversations', () => {
  ...
  it('groups mention notifications by participant set into one public chat', () => {
  ...
  it('different participant sets produce different public chats', () => {
  ...
  it('mentions include other mentioned handles (minus me) in the chat key and handles', () => {
    const alice = makeAccount('2', 'alice');
    ...
    expect(chat.handles.sort()).toEqual(['alice', 'bob']);
  });
  ...
  it('title: joins display names and falls back to handles for mention-only participants', () => {
    ...
    const chat = internals(fixture).chats()[0];
    expect(fixture.componentInstance.title(chat)).toBe('Alice A, @bob');
  });
