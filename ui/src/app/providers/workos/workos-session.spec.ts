...
import { accountPageUrl, displayName, WORKOS_CLIENT_ID, WorkosSession } from './workos-session';

vi.mock('@workos-inc/authkit-js', () => ({ createClient: vi.fn() }));
...
function sessionWith(clientId: string): WorkosSession {
  TestBed.configureTestingModule({
    providers: [{ provide: WORKOS_CLIENT_ID, useValue: clientId }],
  });
...
describe('WorkosSession', () => {
  beforeEach(() => {
    // The Angular unit-test builder runs with `isolate: false`, so spec files
    // share one realm and one module registry. Another file's
    // `vi.restoreAllMocks()` therefore strips the implementation off this
    // module mock, which presented as these tests passing alone and failing in
    // the suite. Re-establishing the spy every test makes the file independent
    // of whatever ran before it. See `src/test-setup.ts` for the same class of
    // bug with `window.location`.
