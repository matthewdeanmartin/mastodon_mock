# Security Policy for `mastodon_mock`
...
1. **GitHub private vulnerability reporting** —
   [submit a confidential report](https://github.com/matthewdeanmartin/mastodon_mock/security/advisories/new)
...
For maximum isolation, run `mastodon_mock` inside a container so it only has access to what you
explicitly mount.
...
docker run --rm -v "$(pwd):/data" mastodon_mock /data
```
