...
def test_tiny_generates_expected_account_and_status_counts(engine):
...
def test_tokens_are_capped_but_accounts_loginable(engine):
...
def test_no_orphan_foreign_keys(engine):
...
def test_relationship_pairs_are_unique(engine):
...
def test_appends_rather_than_replacing(engine):
...
def test_estimate_rows_is_in_the_ballpark(engine):
    cfg = PRESETS["tiny"]
