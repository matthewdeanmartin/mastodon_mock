...
1. dependency-injected authentication and SQLAlchemy session handling in `deps.py`;
1. durable state in `db/models.py` and cross-cutting writes in `services.py`; and
1. the Mastodon-shaped serializer in `serializers/`.
